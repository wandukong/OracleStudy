...
**Outer NL 조인**
 - NL 조인의 특성상 Outer 기호(+)가 붙지 않은 테이블을 항상 드라이빙 테이블로 선택한다.
	- LEADING 이나 ORDERED 힌트 사용해도 순서 변경 X

<hr/>

**Outer 소트 머지 조인**
- 소트 머지 조인은 각 테이블의 조인 대상 집합을 정렬한 후 조인한다는 점이 NL 조인과 다르지만 원리는 NL 조인과 동일
 -  Outer 기호(+)가 붙지 않은 테이블을 항상 드라이빙 테이블로 선택한다.
	- LEADING 이나 ORDERED 힌트 사용해도 순서 변경 X

**Outer 해시 조인**
- 오라클 10g부터 해시 조인에 한해서 조인 방향을 바꿀 수 있다.
- 데이터가 작은 테이블 데이터로 해시 맵을 만들어 부하를 줄일 수 있게 되었다.
- **SWAP_JOIN_INPUTS** 힌트를 사용하여 해시 맵을 어떤 테이블로 만들지 결정
```sql
...
```
스칼라 서브 쿼리?
  입력값에 대한 반환 값을 한 개 만 반환하는 쿼리
```
- 내부적으로 캐시에 값을 저장해 둔다.
	- 입력값과 출력 값이 같으면 쿼리를 수행하지 않는다는 원리로 캐시에 저장
	- 캐쉬에서 입력값을 찾아본다. 만약 있다면, 미리 저장된 출력값 반환
		- 없다면, 스칼라 서브 쿼리 실행해 결과값을 캐시에 저장
- 스칼라 서브 쿼리로의 입력 값이 다양하게 들어오면 캐시에 저장해야 할 값이 많아짐과 동시에 재사용될 가능성도 낮아진다.
	- 재사용 가능성이 낮은 상황에서 캐시를 확인하는 불필요한 과정이 발생
	- 가급적이면 자주 반복되고 값의 종류가 다양하지 않은 테이블과 조인이 필요할 때 사용하면 효과가 좋다.
- 매우 편리한 반면 반환 값을 하나밖에 사용하지 못한다는 단점 존재

**스칼라 서브 쿼리를 두 개 이상 사용해야 할 경우**
- 두 컬럼을 LPAD라는 함수를 사용하여 LPAD(DNAME,20) || LPAD(LOC, 20) 처럼 연결
	- LPAD(컬럼,"총문자길이","채울문자")
		- 채울문자를 지정하지 않은 경우 공백으로 채움
	- LPAD 함수를 사용할 때 주의할 점은 컬럼 길이보다 길게 만들어야 한다.
- LPAD를 사용하면 SUBSTR 함수로 길이를 예측해 컬럼을 다시 나눌 수 있기 때문에, 더 작은 부하로 사용이 가능
```sql
SELECT EMPNO
       , ENAME
       , JOB
...
FROM (
		SELECT E.EMPNO
			   , E.ENAME
			   , E.JOB
			   , (SELECT LPAD(DNAME, 20) || LPAD(LOC,20) 
				  FROM DEPT
				  WHERE DEPTNO = E.DEPTNO) SUB
	    FROM EMP E
);
```